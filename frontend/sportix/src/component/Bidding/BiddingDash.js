...
const BiddingDash = () => {
  ...
  const onBidding = (leagueName, leagueType) => {
    ...
    axios
      .post("http://localhost:8001/startBidding", {
        league: leagueName,
        type: leagueType,
        date: currentDate,
      })
      .then((response) => {
        if (response.data.message) {
          ...
          window.location.href = "/BiddingDash";
        }
      })
      .catch((error) => {
  ...
  const onStopBidding = (leagueName) => {
    axios
      .post("http://localhost:8001/stopBidding", {
        league: leagueName,
      })
      .then((response) => {
        if (response.data.message) {
              ...
              {cric.length >= 0 && (
                <Container maxWidth="lg" sx={{ mt: "40px" }}>
                  <Stack direction="row" justifyContent="space-between">
                    ...
                    <Link to="/BiddingSown" style={{ textDecoration: "none" }}>
