...
import Fron from "../Pictures/frr.jpeg";
...
const BadmintonDash = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const result = await axios(
                'http://localhost:8001/getBadmintonSchedule',
    ...
    return (
        <>
            <Navbar />
      <Header backgroundImage={Fron} title={"Badminton"} title1={"Teams"} />
            {!temp?<>
            {cric.length >= 0 &&
                <Container maxWidth="lg" sx={{ mt: "40px" }}>
                    <Stack direction="row" justifyContent="space-between">
                        <Typography variant="h5" color="#fff"><b>Badminton</b></Typography>
                        <Link to="/BadmintonSchedule" style={{ textDecoration: "none" }}><Button variant="contained"><AddIcon />Add Schedule</Button></Link>
                    </Stack><br />
